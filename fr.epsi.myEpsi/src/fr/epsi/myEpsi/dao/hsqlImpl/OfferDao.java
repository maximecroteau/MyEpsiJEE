...
public class OfferDao {
	...
	private static void logInsert(boolean succes, int id, String titre, String description, int statut, Double prix,
			Date creation, String idVendeur, SQLException e) {
		if (logLevel.actualLogLevel == logLevel.DEBUG) {
			...
			toDebug.append(
					"INSERT INTO ANNONCES (ID , TITLE , CONTENT , PRICE , USER_ID , CREATION_DATE , STATUS , NB_VIEW , UPDATE_DATE) ");
			toDebug.append("VALUES (" + id + " , \"" + titre + "\" , \"" + description + "\" , " + prix + " , \""
					+ idVendeur + "\" , \"" + creation + "\" , " + statut + " , 0 , \"" + creation + "\")");
			if (!succes) {
				toDebug.append(" : " + e);
			}
	...
	public static void saveOffer(Offer newOffer) {
		...
		try {
			...
			PreparedStatement psmt = con.prepareStatement(
					"INSERT INTO ANNONCES (ID,TITLE,CONTENT,PRICE,USER_ID,CREATION_DATE,STATUS,NB_VIEW,UPDATE_DATE) VALUES(?,?,?,?,?,?,?,?,?)");
			...
			logInsert(true, newOffer.getId(), newOffer.getTitre(), newOffer.getDescription(), newOffer.getStatut(),
					newOffer.getPrix(), (Date) newOffer.getCreation(), newOffer.getVendeur().getId(), null);
			con.close();
		} catch (SQLException e) {
			logInsert(false, newOffer.getId(), newOffer.getTitre(), newOffer.getDescription(), newOffer.getStatut(),
					newOffer.getPrix(), (Date) newOffer.getCreation(), newOffer.getVendeur().getId(), e);
		}
	}
